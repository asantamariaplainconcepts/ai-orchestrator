...
- [ ] **The hold label created** (one-time `gh label create`, name from `holdLabel` in `.claude/workflow.json`) — eleven total. Not a `status:*` label and never one of the nine; provisioned by hand like them, because no command may invent it (#323). Until it exists, every command that needs it stops and reports it missing — which is the designed behaviour, not a failure
...
- [x] Canonical system extracted from the A8 reference — its stylesheet fetched and its running app measured, not described. OKLCH tokens on one hue, both themes, one UI kit (one experience per DEC-021), content/voice fundamentals, provenance and deviations cited by ID
- [x] `DESIGN.md` + runtime token adapter generated by a dependency-free generator (both marked generated, regeneration command in header; adapter binds token *names* to `var(--…)`, never copied values)
- [x] Three-stage drift gate wired into the CI lint lane (adherence / drift / skill hygiene)
- [x] Value-free design-router skill written (`.claude/skills/aio-design/`)
- [x] Verify: **probed both ways** — raw hex, raw `37px`, unapproved font, un-regenerated token, and a literal pasted into the skill each FAIL; clean tree passes all three stages. Both themes checked in a real browser for layout, contrast (measured) and focus
- [x] **Synced to main** via `/aio:sync` — retro appended, ADR-0003 written, 3 delta specs folded in (18 living specs), archived as `2026-07-25-design-system`, squash-merged as one commit (`00259ee`), message linted pre-merge, no `[skip ci]` leak, CI green on main
