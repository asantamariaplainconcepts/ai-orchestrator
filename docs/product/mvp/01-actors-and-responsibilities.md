...
| ACT-003 | **Agent** | System actor | The AI execution unit (KEDA-scaled ACA Job). Reads a story through the Connector, produces a plan (phase 1) and/or executes the configured action (phase 2): open PRs, comment, transition, estimate. Acts only inside a Run created by the orchestrator. |
