...
| **Run** | One execution of an Automation against one Story: lifecycle, plan, logs, output link, cost. States: `Queued · Planning · AwaitingApproval · AwaitingInput · Executing · Succeeded · Failed · Cancelled` ([BR-014](05-business-rules.md)). |
| **Plan** | The proposal an Agent produces in phase 1 of an approval-gated Run — reviewed by a human in the website before execution, mirroring spec review ([DEC-040](../mvp/10-locked-mvp-decisions.md)). |
| **Approval** | The human act of marking a Plan ready for execution ([UC-013](04-capabilities.md)). Only exists when the Automation has `requiresApproval` ([DEC-039](../mvp/10-locked-mvp-decisions.md)). |
...
| **Inbox** *(new in v1)* | The one cross-project list of every Run waiting on a human — an approval, an answer, a failure decision ([UC-026](04-capabilities.md)). Subtraction-based: entries leave when acted on. |
