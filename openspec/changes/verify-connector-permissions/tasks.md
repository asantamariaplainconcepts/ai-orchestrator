...
- [ ] 1.1 `Translate` distinguishes a permission refusal from an outage (design D3), carrying the
      vendor's own reason. Four causes, four errors. This lands first because D6 depends on it.
- [ ] 2.1 `VerifyAccess` returns a verdict per capability (design D2) — Stories and documents, each
      with success and the vendor's reason on refusal — implemented for both vendors.
- [ ] 3.1 The probe performs the reads the product performs (design D1) and writes nothing; a
      document path that does not exist is a pass, not a refusal (design D6).
- [ ] 4.1 `ConfigureConnector` refuses naming the capability and the vendor's reason, and stores
      nothing when any capability is refused.
- [ ] 5.1 One query slice for the on-demand test (design D4), calling the same probe (design D5),
      writing nothing.
- [ ] 6.1 The Settings panel offers the test and renders the per-capability result, four states,
      both themes, copy through the i18n catalogue.
- [ ] 7.1 Tests: a token that lists Stories but is refused contents fails the save naming the
