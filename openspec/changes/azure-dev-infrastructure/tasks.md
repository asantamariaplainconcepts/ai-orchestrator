...
- [ ] 0.1 The owner's az login reaches the target subscription: `az account show` names it.
      **Owner action** — at grill time the subscription was invisible to the cached CLI login
      (guest tenants behind MFA). Nothing below starts until this is real.

## 1. State backend

- [ ] 1.1 `infra/bootstrap.sh`: idempotent az CLI creation of `rg-aio-tfstate` + storage account
      + `tfstate` container in northeurope; prints backend config. Running twice changes nothing.
- [ ] 1.2 `.gitignore`: `*.tfstate*`, `*.tfvars`, `.terraform/` — verified by attempting to
...
- [ ] 2.1 `infra/dev/`: provider pinned, subscription guard (fails plan on any subscription other
      than the expected one — expressed without committing the id), resource group, Log
      Analytics.
- [ ] 2.2 Container registry (Basic), Container Apps environment (Consumption) wired to Log
      Analytics.
- [ ] 2.3 PostgreSQL Flexible Server (B_Standard_B1ms/32 GB), `aiorchestratordb` database,
      firewall stance recorded in-code (dev: Azure-services access for ACA; no public 0.0.0.0).
- [ ] 2.4 Key Vault (standard, RBAC mode). Terraform-generated database credentials stored as
      vault secrets; nothing sensitive in non-sensitive outputs (`terraform output` inspected).
- [ ] 2.5 Portal container app: system-assigned identity, AcrPull + Key Vault Secrets User role
...
- [ ] 3.1 `KeyVaultSecretResolver` in BuildingBlocks beside the configuration resolver;
      ...
      unchanged.
- [ ] 3.2 Unit-test the selection rule; functional tier keeps the configuration resolver (a test
      must not require a cloud — ADR-0002).
- [ ] 3.3 Dockerfiles/publish for Server and MigrationService images; build and push to ACR.
- [ ] 3.4 `infra/deploy.sh`: push image → run migration job → wait for exit 0 → update app
...
- [ ] 4.1 `terraform` lane: fmt-check + validate on PRs touching `infra/`; fails loudly when it
      cannot run — never a silent skip.

## 5. Close-out

- [ ] 5.1 `ARCHITECTURE.md`: deployment topology section; the deploy sequence and who runs it.
