...
- [ ] 1.1 `AiOrchestrator.Modules.Projects.Contracts`: `IAutomationCatalog` (enabled Automations
      of a Project: id, trigger label/state, requiresApproval) — no implementation types.
      Projects registers the implementation.
- [ ] 1.2 `IStoryReader` in `AiOrchestrator.Modules.Backlog.Contracts` (labels + state by
      project id and vendor story id). Backlog registers the implementation.
- [ ] 1.3 Guardrails re-run with both in place: boundary ArchTests green, no vacuous pass
      (the allowed-direction test must now see three Contracts references).

## 2. The Runs module

- [ ] 2.1 Module skeleton: `AiOrchestrator.Modules.Runs`, schema `runs`, migration, discovery
...
- [ ] 2.2 BR-001 partial unique index over the story reference across active states — in the
      initial migration, not a follow-up.
- [ ] 2.3 The `StoryChanged` handler: read Story via `IStoryReader`, match against
...
- [ ] 2.4 Enqueue via `IRunDispatcher` after commit (design D4); enqueue failure logs loudly
      and leaves the `Queued` Run visible.

## 3. Tests

- [ ] 3.1 Functional (real containers, real CAP relay): the loop-closes scenario — configure
      backlog, add Automation, label a Story, refresh → a Run exists and the queue holds its id.
- [ ] 3.2 Functional negatives with the fence pattern (#41 retro): no match → nothing;
...
- [ ] 3.3 Concurrency (ADR-0002): parallel deliveries for the same Story → exactly one Run —
      the index decides, both handlers report success.
- [ ] 3.4 ArchTests as task 1.3; analyzers untouched.

## 4. Close-out

- [ ] 4.1 ARCHITECTURE.md: Runs section — where matching lives, what the stated limitations are.
