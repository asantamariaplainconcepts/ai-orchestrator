...
- [ ] 1.2 Add `"holdLabel": "hitl"` to `.claude/workflow.json`, with a `$comment`-style note (matching
...
- [ ] 1.3 Verify AC 1 mechanically: `grep -rIn 'hitl' --exclude-dir=node_modules --exclude-dir=.git .`
...
- [ ] 2.1 In `.claude/skills/read-issue/SKILL.md`, extend step 2 to extract the hold from the `labels`
  already fetched, comparing case-insensitively, resolving its name from `holdLabel` in
  `.claude/workflow.json` (AC 4, AC 3).
- [ ] 2.2 In the same file, state that the returned result reports the hold as present **or absent** —
  never omitted — and that the skill stays read-only: it neither applies nor removes the hold.
- [ ] 2.3 In `.claude/skills/set-issue-status/SKILL.md`, add to the lifecycle reference that the hold
...
- [ ] 3.1 `.claude/commands/aio/propose.md`: add the hold check as the gate immediately after
  `read-issue`, before any git operation — refuse naming the hold and who clears it, creating no
  branch and no PR (AC 5).
- [ ] 3.2 `.claude/commands/aio/propose.md`: change the final advance from
  ...
  (AC 12).
- [ ] 3.3 `.claude/commands/aio/implement.md`: add the hold check as step 2.5 — after `read-issue`,
...
- [ ] 3.4 `.claude/commands/aio/implement.md`: change the final advance so `status:code-review` and
...
- [ ] 3.5 `.claude/commands/aio/sync.md`: add the hold check as the first gate in step 2, before the
  draft/rollup checks — refuse so nothing is merged, archived, or appended to the retro log (AC 7).
- [ ] 3.6 `.claude/commands/aio/grill.md`: state that on a held issue it still evaluates and may
  comment, and calls no `set-issue-status`, reporting the hold as the reason the status did not
  advance (AC 8, D5).
- [ ] 3.7 `.claude/commands/aio/status.md`: report the hold alongside the lifecycle position, name
...
- [ ] 3.8 `.claude/commands/aio/refine.md`: state that the hold does not affect it — it is post-merge
  and gates nothing (AC 10, D5).
- [ ] 3.9 Confirm no command or skill file removes the hold label anywhere:
  `grep -rn 'remove-label' .claude/` names only `status:*` labels.

## 4. Documentation and traceability

- [ ] 4.1 `CONTRIBUTING.md`: update the lifecycle diagram and the stage table so both HITL stages are
...
- [ ] 4.2 `AGENTS.md`: update the lifecycle line to match.
- [ ] 4.3 `docs/process/`: update whichever process document describes the two review stages so it
  agrees with `CONTRIBUTING.md`.
- [ ] 4.4 `BOOTSTRAP-CHECKLIST.md` (and `BOOTSTRAP.md` where it lists label provisioning): add the
  one-time hold-label creation from task 1.1 alongside the nine `status:*` labels.
- [ ] 4.5 `docs/product/mvp/09-foundation-vs-product-split.md`: add a Foundation row for the `/aio:*`
...
- [ ] 5.1 Apply the task-3 edits to the six byte-identical copies under
...
- [ ] 5.2 Verify equality is restored: `diff` each pair against `.claude/commands/aio/` and expect no
  output for all six.
- [ ] 5.3 Leave the manifest's `prerequisites` block unchanged, and carry Open Question 1
  (`.claude/workflow.json` as a prerequisite) to the reviewer rather than resolving it here.

## 6. Verification

- [ ] 6.1 Re-run the AC 1 grep from task 1.3 after all edits — still only `workflow.json` and this
  change's bundle.
- [ ] 6.2 Walk each of ACs 1–14 against the edited files and record the file:line that satisfies it,
...
- [ ] 6.5 Run the repository's CI-equivalent gates for the touched surfaces:
...
- [ ] 6.6 Confirm the commit messages pass commitlint (`docs:`/`feat:` per the surface touched) and
