...
- [x] 1.1 A hosted service in the Runs module, composed in the long-lived host and not in the
      dispatch worker (design D4), sweeping on a configurable interval that is not hardcoded.
- [x] 2.1 The overdue query: `Planning`/`Executing` Runs whose `StartedAt` plus the Automation's
      timeout plus a grace period has passed (design D1). No new column — `StartedAt` exists.
- [x] 3.1 The write is conditional on the observed state (design D5), so a Run that finished in
      between keeps its outcome. A grace period is a guess; this is the guarantee.
- [x] 4.1 A failure reason that names a worker that never reported, distinct from the executor's
      own timeout message (design D2).
- [x] 5.1 Tests: an overdue Run is failed and its Story freed; a Run inside its deadline is
