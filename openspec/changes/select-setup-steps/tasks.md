...
- [x] 3.1 Mirror both DTO changes in `useWorkflowSetup.ts`: `outputLabels: string[]` on
      `PlannedStep`, `steps?: string[]` on `WorkflowSetupInput`, `excluded: string[]` on
      `WorkflowSetupReport`.
- [x] 3.2 Hold selection state in `WorkflowSetupSection`, seeded to every row of the shown
      ...
      nobody chose.
- [x] 3.3 Render a selection control per row in `Plan`, labelled from the i18n catalogue (DEC-021 —
      hardcoded JSX copy fails CI). The row stays readable when deselected; it is excluded, not
      gone.
- [x] 3.4 Write the hand-off function: a pure function over plan rows returning, for the current
...
- [x] 3.5 Mark the gap on the affected row and leave the confirm enabled. Disable the confirm only
      when the selection is empty.
- [x] 3.6 Send `steps` from the confirm; leave `installMissing: true` as it is.
- [x] 3.7 Render `excluded` in the report as its own fact, beside `skipped` and distinct from it.
- [x] 3.8 Add the new copy to `en.ts`. Remove the orphaned `workflowSetup.installMissing` key
      (verified unreferenced — #233 deleted the control it belonged to).

## 4. Keep mock mode honest

- [x] 4.1 The mock's `discover-pipeline` candidates carry no `plan` at all (`mock.ts:445-461`), so
...
- [x] 4.2 Have the mock's `set-up-defaults` honour `steps` and answer with `excluded`, so mock mode
      shows the same report shape the API returns.

## 5. Tests

- [x] 5.1 Functional (`PipelineAdoption_Should_Constraint`): an absent selection creates every step;
...
- [x] 5.2 Functional: a partial selection creates exactly the selected steps, and the excluded ones
      appear in `Excluded` and nowhere else.
- [x] 5.3 Functional: an excluded gap is absent from the published files
      (`StubInstallWorkspace.PublishedFiles`), while the selected gaps are present.
- [x] 5.4 Functional: excluding every gap leaves `PreparedBranch` unset, opens no pull request, and
      reports no failure — the trap in 2.4, asserted rather than assumed.
- [x] 5.5 Functional: a step excluded by the selection whose trigger the project also already uses
      appears only in `Excluded`, never in both lists.
- [x] 5.6 Functional: a selection naming a trigger in a different case selects the step; a selection
      naming an unknown trigger succeeds and creates nothing from it.
- [x] 5.7 Functional: the discovery plan carries each step's output labels, and computing it still
      costs no additional vendor read.
- [x] 5.8 **Stated, not faked:** no E2E test for the selection surface.
      ...
      reach the state.
- [x] 5.9 **Stated, not faked:** the hand-off function has no unit test, because
...
- [x] 5.10 Mutation check on the load-bearing assertions: making the filter case-sensitive, treating
...
- [x] 6.1 `dotnet build`, then the non-E2E suite, then the E2E suite.
- [x] 6.2 CSharpier, Prettier, ESLint `--max-warnings=0`, `tsc --noEmit`, the design-system
      validator.
- [x] 6.3 Frontend production build — run through `rtk proxy`, not `rtk`. This repository has had a
...
- [x] 6.4 `openspec validate select-setup-steps`.
