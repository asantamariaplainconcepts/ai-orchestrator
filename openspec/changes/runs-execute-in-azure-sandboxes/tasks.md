...
- [ ] 0.1 A subscription where `Microsoft.App/sandboxGroups` can be **created**, not merely read.
...
- [ ] 1.1 `AcaAgentProcessHost` implements `IAgentProcessHost`, selected by
...
- [ ] 1.2 `Run()` starts the agent **detached** inside the sandbox and polls with short `exec`
...
- [ ] 1.3 BR-005's timeout kills the phase and BR-004 still never retries. The sandbox is disposed
      in a `finally` that survives cancellation, like sbx's.
- [ ] 1.4 Choose and defend the poll interval (design's open question): live enough for UC-027,
      sparse enough not to spend an `exec` per second across thirty minutes.

## 2. What the habitat declares (design D3)

- [ ] 2.1 The launcher disables auto-suspend on every sandbox it creates. A test pins it, because
...
- [ ] 2.2 Egress is declared deny-default with an allow list, and composition **refuses** a habitat
...
- [ ] 3.1 The Run's workspace reaches the sandbox without the executor sharing its machine.
...
- [ ] 5.1 The port is created **Entra-gated**, never `--anonymous`, and the portal relays it as it
