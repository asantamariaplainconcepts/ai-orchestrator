...
- [ ] 1.1 Rebase this branch onto #331 (`local-run-in-its-own-checkout`) once it lands, and confirm
...
- [ ] 1.2 Amend UC-031's entry in `docs/product/v1/04-capabilities.md` with one sentence
...
- [ ] 2.1 Add `ILocalCheckoutSetup` to `src/shared/AiOrchestrator.BuildingBlocks/Agents/`: run one
...
- [ ] 2.2 Implement it in `src/shared/AiOrchestrator.Infrastructure/Agents/` over the existing
...
- [ ] 2.3 Add `LocalSetupErrors` in the `LocalWorkspaceErrors` pattern: distinct codes and sentences
...
- [ ] 2.4 Unit-test the seam directly against real child processes: a zero exit, a non-zero exit, a
...
- [ ] 3.1 Add the nullable setup command to `Connector`
      ...
      (design D9).
- [ ] 3.2 Add one additive EF Core migration on the Backlog schema — a nullable column, no backfill.
      Verify an existing database migrates and every row reads as having none.
- [ ] 3.3 Extend `ConfigureConnector`'s request, response, command and validator: bounded length,
...
- [ ] 3.4 Expose the field on `IConnectorReader`/`ConnectorReader`
...
- [ ] 3.5 Functional tests in `AiOrchestrator.Modules.Backlog.FunctionalTests`: stored and returned
...
- [ ] 4.1 In `RunExecutor`'s Local branch, start the phase clock from the injected `TimeProvider`
      ...
      clock.
- [ ] 4.2 After `RecordLocalExecution` and before the runtime, write the header line naming the
      command through `onOutput`, then run setup through `ILocalCheckoutSetup` with the remaining
      budget (design D6, D7).
- [ ] 4.3 Map the three outcomes: zero exit continues to the runtime; non-zero fails the Run with the
      setup refusal before any runtime invocation; a timeout fails naming the limit, never naming a
      setup failure.
- [ ] 4.4 Call `Conclude(succeeded: false)` on the setup-failure and overrun paths so the checkout is
      removed exactly as on any other failure (design D7, spec "a failed setup removes its
      checkout").
- [ ] 4.5 Skip the whole block when no command is configured — no process, no line, straight to the
      runtime (spec "no command configured runs nothing").

## 5. Surface

- [ ] 5.1 Add the setup-command input to
      ...
      `DESIGN.md` (design D9).
- [ ] 5.2 Add its label, placeholder and explanation to `src/frontend/shared/i18n/en.ts`; no
      hardcoded JSX copy (DEC-009, DEC-021).
- [ ] 5.3 Thread the value through `ProjectScreen.tsx`/`useBacklog.ts` and send it as null whenever
      the code source is `Repository`; update `src/frontend/shared/http/mock.ts`.
- [ ] 5.4 Confirm the Advanced disclosure opens by itself for a Connector that stores a command
      (spec "a stored command opens the disclosure").

## 6. Behavioural coverage

- [ ] 6.1 Functional test: a Local Run with a configured command runs it to completion in the Run's
      checkout before the runtime is invoked (acceptance criterion 1).
- [ ] 6.2 Functional test: a non-zero exit ends the Run `Failed` naming the setup, the command and
      its output, with the runtime never invoked (criterion 2, BR-004).
- [ ] 6.3 Functional test: no command configured starts no process and invokes the Agent immediately
      (criterion 3).
- [ ] 6.4 Functional test: a command still running at the phase timeout is killed and the Run names
      the limit, not a setup failure (criterion 4, BR-005) — and a completed setup leaves the runtime
      bounded by the remainder.
- [ ] 6.5 Functional test: the setup's output precedes the Agent's in the Run's log, and the header
      line naming the command is readable before the command's own output (criterion 5, UC-027,
      BR-014).
- [ ] 6.6 Test that no file in the checkout is read or executed as setup (spec "nothing in the
      checkout can become the command").

## 7. Exercised for real, not assumed

- [ ] 7.1 Run a real Local Run end to end against a checkout of this repository with the setup
...
- [ ] 7.2 Confirm on the running Server that the shell resolves the same toolchain the Agent process
...
- [ ] 8.1 `dotnet build` clean with no new analyzer warnings (MOD001–005, CQS001), and CSharpier
      formatting applied.
- [ ] 8.2 `dotnet test` green, including the ArchTests and the new functional tests.
- [ ] 8.3 Frontend: Prettier, `eslint --max-warnings=0`, `tsc --noEmit`, and the production
      `pnpm build` — the E2E suite serves the built bundle, so an unbuilt `.tsx` edit is invisible to
      it.
- [ ] 8.4 `openspec validate local-run-checkout-is-ready-to-build --strict` passes and the change is
      ready to archive.
