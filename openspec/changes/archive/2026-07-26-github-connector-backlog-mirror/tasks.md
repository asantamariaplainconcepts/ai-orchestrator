...
- [x] 1.1 `ISecretResolver` in `AiOrchestrator.BuildingBlocks`: resolve a secret **by name, per
      read**, failing loudly when it is missing — never empty, never a default, never cached in a
      way that survives a rotation.
- [x] 1.2 A configuration-backed implementation for development (user-secrets / environment),
...
- [x] 1.3 The Key Vault implementation is **not written here** — it belongs to #8, which is where a
...
- [x] 1.4 Verify: resolving a missing name throws with the name in the message; a secret added
...
- [x] 2.1 `AiOrchestrator.Modules.Backlog` with its own `backlog` schema, `DbContext`, migration
...
- [x] 2.2 **Verify the module seam for real, now that two modules exist:** the solution builds
...
- [x] 2.3 Verify: the Backlog migration touches only the `backlog` schema.

## 3. The connector seam and its GitHub implementation

- [x] 3.1 `IBacklogConnector`: verify access, and fetch current Stories. Product vocabulary only —
      no Octokit type in the signature.
- [x] 3.2 The GitHub implementation over Octokit 14: coordinate + credential verification with the
      two failure modes distinguished, and Story retrieval with pagination. **Conditional requests
      are not implemented:** Octokit 14 exposes `ApiInfo.Etag` for reading but offers no way to
      send `If-None-Match` and no not-modified result, so there is nothing to build on. The spec
      was amended to drop the change token rather than describe a behaviour the client cannot
      have.
- [x] 3.3 Verify: a deliberate bad-credential and a deliberate bad-repository case each produce
      their own distinct failure, not one generic error.

## 4. Configuring a Connector (UC-004)

- [x] 4.1 `ConfigureConnector` slice: validator, handler, `ErrorOr` domain errors in a
      `{Entity}Errors` type, verification-before-store, upsert semantics (one Connector per
      Project).
- [x] 4.2 A read slice returning coordinates and secret name — never a token. Implemented as
      `GetBacklog` rather than a separate `GetConnector`: the page needs the Connector's health
      and its Stories together, and two endpoints would let a client render them from two
      moments in time.
- [x] 4.3 Verify: the stored row contains no token; the API response contains no token; a rejected
      credential and an unknown repository return different problems.

## 5. Polling and the mirror (UC-009)

- [x] 5.1 Reconciliation: upsert by vendor id, mark absent what the vendor no longer returns,
      identity independent of title.
- [x] 5.2 `RefreshBacklog` slice — the explicit, deterministic path the tests drive.
- [x] 5.3 The hosted-service poller on the per-project interval (default 60s). It must not delay
...
- [x] 5.4 Failure handling: record the failure and its reason against the Connector; leave the
      previous mirror readable; make "no Stories" and "last poll failed" distinguishable.
- [x] 5.5 Verify: two consecutive polls with nothing changed leave the mirror byte-identical;
      a poll against an unreachable vendor leaves prior Stories readable and records the failure.

## 6. The project page (UC-007)

- [x] 6.0 **Route this work through the `aio-design` skill**: read `DESIGN.md` first, compose kit
      components, resolve copy through the catalogue, run the validator before pushing.
- [x] 6.1 A project route showing the Connector configuration form and the mirrored Stories, built
...
- [x] 6.2 If the page genuinely needs something the kit lacks, **add it to
...
- [x] 6.3 All copy through the typed i18n catalogue, following the content fundamentals: sentence
...
- [x] 6.4 Verify: `pnpm lint`, `pnpm typecheck` and
...
- [x] 7.1 Unit: reconciliation logic (add / update / disappear / rename), and the failure-mode
      mapping.
- [x] 7.2 Functional: configure → poll → read, against real containers, with the vendor stubbed at
      the `IBacklogConnector` seam so the tier stays hermetic and does not depend on GitHub.
- [x] 7.3 A concurrency test, per ADR-0002 — parallel refreshes of the same Project must not
      duplicate Stories.
- [x] 7.4 E2E: the project page shows a mirrored backlog. Whether the vendor is stubbed at the
...
- [x] 8.1 `ARCHITECTURE.md` and the module `context.md` updated: two modules now, and *why*
      Backlog owns the Connector (design D2) so the next reader does not "fix" it.
- [x] 8.2 Record the referential-integrity debt from D2 where project deletion will have to deal
      with it.
- [x] 8.3 Full verify sweep; CI green including E2E.
