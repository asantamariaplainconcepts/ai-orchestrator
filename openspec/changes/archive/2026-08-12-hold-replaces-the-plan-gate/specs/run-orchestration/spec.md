...
**Reason**: its scenario "the gate is not a bypass" asserted that Run now refuses a `requiresApproval` Automation. That flag is gone (DEC-067), so the scenario names a configuration that cannot exist. BR-013's principle is unchanged and is restated below against the hold.

**Migration**: the same guarantee, against the rule that replaced it: Run now refuses a held Story and names the hold.

### Requirement: a matching story event creates a Run and dispatches it


**Reason**: the requirement branched on `requiresApproval` and its scenario asserted that the two-phase lane is refused. There is no such lane after DEC-067 — every Run is single-phase — so the scenario asserts a refusal of something that cannot be configured. Restated below with the hold's refusal in its place.

**Migration**: an Automation that used the two-phase lane marks the hold on success instead; the refusal a reader now meets is the held Story, and it is stated in the restated requirement.
