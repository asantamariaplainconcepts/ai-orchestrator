...
**Reason**: the requirement said the executing steps "SHALL require approval" and its scenario asserted the tab draws three approval gates. Both describe the flag DEC-067 removed.

**Migration**: the same three stops, spelled as the hold among each step's marks: propose, implement and sync each hold the Story when they finish, so the chain still stops three times — after each step acts rather than inside it.

## ADDED Requirements

### Requirement: the spec-first tier arrives as one chain that holds for a person
