...
- [ ] 1.1 Rename `ReadyLabel` to `OutputLabel` across the domain, the Contracts surface, the
      ...
      before.
- [ ] 2.1 The executor writes it once, on success, for every action (design D2); the grill's
      code default applies only to the grill action.
- [ ] 3.1 The self-trigger refusal at save (design D3), with its sentence.
- [ ] 4.1 Tests: the chain for a non-grill action, silence when unset, nothing on
